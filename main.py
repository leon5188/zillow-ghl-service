...
async def enrich_contact(request: EnrichmentRequest, background_tasks: BackgroundTasks):
    logger.info(f"Received enrichment request for {request.contact_id} at {request.address}")
    
    # Run scraping and update in background to return immediate response to GHL
    background_tasks.add_task(process_enrichment, request.contact_id, request.location_id, request.address)
